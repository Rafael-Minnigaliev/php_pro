...
if($userId['user_id'] != 0){
    $count = DB::getRow("SELECT sum(count) c_sum FROM cart WHERE users_id = :id AND status = 0", ['id' => $userId['user_id']]);
    $totalPrice = DB::getRow("SELECT sum(price * cart.count) p_sum FROM cart JOIN goods ON cart.goods_id = goods.id WHERE users_id = :id AND status = 0", ['id' => $userId['user_id']]);
}else{
