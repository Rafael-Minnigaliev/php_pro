...
class CUser extends CBase{
    protected function actionProfile(){
        if ($_SESSION['user_id']) {
            ...
            $this->content = $this->template("v_profile.html", array('data' => $data, 'admin' => $_SESSION['role']));
        } else {
    ...
    protected function actionLogin(){
        if (!$_SESSION['user_id']){
            if($this->isMethod('POST')){
                ...
                $this->content = $this->template("v_login.html", array('status' => $_GET['status']));
            }
        } else {
            header("Location: index.php");
        }
    ...
    protected function actionReg(){
        if (!$_SESSION['user_id']){
            if($this->isMethod('POST')){
                ...
                $this->content = $this->template("v_reg.html", array('status' => $_GET['status']));
            }
        } else {
            header("Location: index.php");
        }
