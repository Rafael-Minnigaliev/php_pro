...
WHERE users_id = :id ORDER BY date DESC", ['id' => $id]);
    ...
    public static function getOrderUserInfo($id){
        return DB::getRow("SELECT telephone, address FROM users WHERE id = :id", ['id' => $_SESSION['user_id']]);
    }

    public static function getOrderRegInfo(){
        if($_SESSION['user_id']){
            return DB::Select("SELECT name, price, count FROM cart JOIN goods ON cart.goods_id = goods.id WHERE users_id = :id AND status = 0", ['id' => $_SESSION['user_id']]);
        }else{
    ...
    public static function orderRegistration($payMethod){
        if($_SESSION['user_id']){
            $var = true;
            DB::connect()->beginTransaction();
            DB::insert("INSERT INTO orders(users_id, pay_method) VALUES(:id, :pm)", ['id' => $_SESSION['user_id'], 'pm' => $payMethod]);
            $orderId = DB::getRow("SELECT id FROM orders WHERE users_id = :id ORDER BY date DESC LIMIT 1", ['id' => $_SESSION['user_id']]);
            $update = DB::update("UPDATE cart SET status = 1, order_id = :oid WHERE users_id = :uid AND status = 0", ['oid' => $orderId['id'], 'uid' => $_SESSION['user_id']]);
            if(!$update){
                $var = false;
            }
            if($var){
